...
- API mode with `NEXT_PUBLIC_USE_API=true` has no Team/HR toggle; unauthenticated users hit `/login`. Do not use this feature file against API mode.
- Driving an existing Chrome profile shares `office-hub-demo` localStorage with other tabs on port 3000.
